 ...
 */
export function HomeList({ items, padding }: Readonly<HomeItemsProps>): ReactNode {
	const groupRef = useRef<Frame>();

	// const lengthItems = items.size();
		...
		>
			{items.map((props: ItemProps, index: number) => {
				return (
					<HomeButton
						key={props.text}
						endPosition={UDim2.fromScale(
							DEFAULT_WIDTH + padding.X.Scale * index,
